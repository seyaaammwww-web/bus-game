...
export function RetroQuote({ children, className, title, variant = 'yellow', ...props }: RetroQuoteProps) {
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-[3px] border border-[#2C0834]/10 pointer-events-none" />

            <QuoteArrows />

            {title && (
                <div className="mb-2 font-pixel-title text-sm uppercase tracking-widest text-[#2C0834]/60">
                    {title}
                </div>
            )}

            <div className="relative z-10 text-[#31093A] font-pixel-text leading-relaxed">
                {children}
            </div>
        </motion.div>
    )
